...
缺口继续追问。当前版本基于用户对话、提交材料、经人工审核的官方法规和指导性案例
...
- 按法条编号、词法和可选向量检索人工核验的官方法规，并保存可跳转原文的
  `[L...:C...]` 引用；
- 检索已审核的最高法指导性案例，并单独保存可跳转原文的 `[C...:S...]` 类案引用；
- 明确输出争议焦点、双方论证、证据评价、信息缺口和后续核查事项；
- 对材料数量、单份大小和总上下文设置确定性限制；
- 提供可替换的分析网关，测试不依赖真实模型。

当前检索针对案件中已保存的材料、已审核法规和首批指导性案例，案件档案会作为用户确认的
...
[类案来源与审核](./docs/case-law-sources.md)。本机生产启动方式见
