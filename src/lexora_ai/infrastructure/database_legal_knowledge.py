    ...
    async def _search(
        ...
        *,
        ...
        async with self._session_factory() as session:
            repository = LexoraUnitOfWork(session).legal_sources
            ...
            if not postgres or not vector_search:
                ...
                chunks = await repository.list_effective_chunks(
                    include_embeddings=vector_search,
                    limit=scan_limit + 1,
                )
                if len(chunks) > scan_limit:
                    raise RuntimeError(
                        "legal retrieval corpus exceeds the bounded in-process scan limit"
                    )
            ...
            lightweight_chunks = await repository.list_effective_chunks(
                include_embeddings=False,
                limit=_LEXICAL_CORPUS_LIMIT + 1,
            )
            if len(lightweight_chunks) > _LEXICAL_CORPUS_LIMIT:
                raise RuntimeError(
                    "legal retrieval corpus exceeds the bounded lexical scan limit"
                )
            lexical_candidates = rank_legal_knowledge(
                query,
                lightweight_chunks,
            ...
            vector_candidate_ids = await repository.list_effective_vector_candidate_ids(
                query_embedding,
                embedding_model,
                limit=candidate_k,
            )
            ...
            chunks = await repository.list_effective_chunks(chunk_ids=candidate_ids)
        return rank_legal_knowledge(
            query,
            chunks,
            query_embedding=query_embedding,
